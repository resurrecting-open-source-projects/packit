...
#define icmp_iphdr icmphdr->dun.ip.idi_ip
	...
	case ICMP_REDIRECT:
		...
		if (g_verbose) {
			...
			fprintf(stdout,
				...
				s_addr, d_addr);
			fprintf(stdout,
				"\t     TTL: %d  ID: %d  TOS: 0x%X  Len: %d  ",
				icmp_iphdr.ip_ttl, ntohs(icmp_iphdr.ip_id),
				icmp_iphdr.ip_tos, ntohs(icmp_iphdr.ip_len));
	...
	case ICMP_UNREACH:
		...
		if (icmphdr->icmp_code == ICMP_UNREACH_NEEDFRAG)
			fprintf(stdout, "MTU: %d  Pad: %d  ",
				ntohs(icmphdr->hun.frag.mtu),
				ntohs(icmphdr->hun.frag.pad));
		if (g_verbose) {
			s_addr = libnet_addr2name4(icmp_iphdr.ip_src.s_addr, 0);
			d_addr = libnet_addr2name4(icmp_iphdr.ip_dst.s_addr, 0);
			fprintf(stdout, "\n");
	...
	case ICMP_TSTAMPREPLY:
		...
		fprintf(stdout, "ID: %d  Seqn: %d\n", icmphdr->icmp_id,
			icmphdr->icmp_seq);
		fprintf(stdout,
			"\t     Original: %lu  Received: %lu  Transmit: %lu",
	...
	case ICMP_MASKREPLY:
		...
		fprintf(stdout, "\t     Mask: %s  ",
			libnet_addr2name4(ntohl(icmphdr->dun.mask), 0));
		break;
	case ICMP_ECHOREPLY:
	case ICMP_ECHO:
	case ICMP_MASKREQ:
	case ICMP_TSTAMP:
		if (icmphdr->icmp_code > 0)
			fprintf(stdout, "Code: %s(%d)  ", icmp_c,
				icmphdr->icmp_code);
		fprintf(stdout, "ID: %d  Seqn: %d", icmphdr->icmp_id,
			icmphdr->icmp_seq);
		break;
