...
print_injection_details()
{
    ...
    if(g_injection_type == ETHERTYPE_IP)
    {
        ...
        if(g_ip4hdr_o.p == IPPROTO_TCP && !g_rawip)
        {
            ...
            fprintf(stdout, "TCP header:  Src Port: %d  ", g_s_port);

            if(g_p_mode == M_INJECT_RESPONSE)
                fprintf(stdout, "Dst Port: %d  ", g_d_port);
            else
                fprintf(stdout, "Dst Port(s): %s  ", g_s_d_port);
        ...
        if(g_ip4hdr_o.p == IPPROTO_UDP && !g_rawip)
        {
        ...
        if(g_ip4hdr_o.p == IPPROTO_ICMP && !g_rawip)
        {
	    icmp_t = retrieve_icmp_type(g_i4hdr_o.type);

	    fprintf(stdout, "ICMP header: Type: %s(%d)  ", icmp_t, g_i4hdr_o.type);

	    if(g_i4hdr_o.code > 0)
	    {
                icmp_c = retrieve_icmp_code(g_i4hdr_o.type, g_i4hdr_o.code);

	        fprintf(stdout, "Code: %s(%d) ", icmp_c, g_i4hdr_o.code);
	    }

	    switch(g_i4hdr_o.type)
	    {
		case ICMP_ECHOREPLY: case ICMP_ECHO: case ICMP_TSTAMP:
	            fprintf(stdout, "ID: %d  Seqn: %d  ", g_i4hdr_o.id, g_i4hdr_o.seqn);
		    break;
		...
		case ICMP_UNREACH: case ICMP_REDIRECT: case ICMP_TIMXCEED:
		    ...
		    if(g_i4hdr_o.orig_p == IPPROTO_UDP)
		        fprintf(stdout, "\t     Protocol: UDP(%d)  ", g_i4hdr_o.orig_p);
		    else
                    if(g_i4hdr_o.orig_p == IPPROTO_TCP)
			fprintf(stdout, "\t     Protocol: TCP(%d)  ", g_i4hdr_o.orig_p);
		    else
                    if(g_i4hdr_o.orig_p == IPPROTO_ICMP)
			fprintf(stdout, "\t     Protocol: ICMP(%d)  ", g_i4hdr_o.orig_p);

		    fprintf(stdout, "Src Port: %d  Dst port: %d\n",
	                g_i4hdr_o.orig_s_port, g_i4hdr_o.orig_d_port);

		    fprintf(stdout, "\t     Src Address: %s  Dst Address: %s\n",
	                g_i4hdr_o.orig_s_addr, g_i4hdr_o.orig_d_addr);

	            fprintf(stdout, "\t     TTL: %d  ID: %d  TOS: 0x%X",
			g_i4hdr_o.orig_ttl, g_i4hdr_o.orig_id, g_i4hdr_o.orig_tos);
		...
		case ICMP_TSTAMPREPLY:
		    ...
		    fprintf(stdout, "\t     Transmit Timestamp: %d", g_i4hdr_o.ttime);
	...
	fprintf(stdout, "\t     TTL: %d  ID: %d  TOS: 0x%X  Len: %d  ",
	    g_ip4hdr_o.ttl, g_ip4hdr_o.id, (u_int8_t)g_ip4hdr_o.tos, g_hdr_len);
       ...
       || g_injection_type == ETHERTYPE_REVARP)
    {
       ...
       fprintf(stdout, "DEBUG: %s\n",
               (g_injection_type == ETHERTYPE_REVARP) ? "ETHERTYPE_REVARP" : "ETHERTYPE_ARP");
#endif
