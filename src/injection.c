...
injection_init()
{
    ...
    if(g_s_d_port != NULL)
    {
        if(strstr((char*)g_s_d_port, "-"))
        {
            ...
            if(g_cnt < 1 || g_cnt > 65535)
                fatal_error("Invalid port range: %s", g_s_d_port);
    ...
    if(g_p_mode == M_INJECT_RESPONSE || g_p_mode == M_TRACE)
...
u_int16_t
with_response(u_int32_t g_port_range)
{
    char ufilter[1024];
    ...
    if((g_pkt = pcap_open_live(g_device, 1500, 1, READ_TIMEOUT, error_buf)) == NULL)
    ...
    for(i = 1; i < g_cnt + 1; i++)
    {
        ...
        switch(g_ip4hdr_o.p)
        {
             case IPPROTO_TCP:
                 snprintf(ufilter, 1024,
                     ...
                     g_ip4hdr_o.s_addr, g_ip4hdr_o.d_addr, g_d_port, g_s_port, g_ip4hdr_o.id);
             ...
             case IPPROTO_ICMP:
                 snprintf(ufilter, 1024,
                     ...
                     " || "
                     "((icmp[0] = 0xb || icmp[0] = 0x3)  && icmp[12:2] = 0x%04x))",
                     g_ip4hdr_o.s_addr, htons(g_i4hdr_o.id), htons(g_i4hdr_o.seqn),
                     g_i4hdr_o.id, g_i4hdr_o.seqn, g_ip4hdr_o.id);
       ...
       fprintf(stdout, "DEBUG: pcap_setnonblock()\n");
        ...
        print_separator(1, 2, (g_p_mode == M_TRACE) ? "HOP %d" : "SND %d", g_inj_cnt);
...
start:
        ...
        while(1)
        {
            dth_r = pcap_dispatch(g_pkt, 1, (pcap_handler)process_packets, NULL);
            ...
            if(((g_af_pcap.tv_sec - g_bf_pcap.tv_sec) == g_r_timeout &&
                (g_bf_pcap.tv_usec < g_af_pcap.tv_usec)) ||
                (g_af_pcap.tv_sec - g_bf_pcap.tv_sec) > g_r_timeout)
            {
                ...
                {
                    tr_retry = 0;
                    print_separator((g_p_mode == M_TRACE) ? 0 : 1, 1, "No Response From Peer");
                    break;
                }
            }
        ...
        if((g_ip4hdr_o.p == IPPROTO_TCP || g_ip4hdr_o.p == IPPROTO_UDP) && g_port_range)
            g_d_port++;
        else
        if(g_ip4hdr_o.p == IPPROTO_ICMP && g_i4hdr_o.type == ICMP_ECHO)
            g_i4hdr_o.seqn++;

        if(g_burst_rate != 0 && g_p_mode != M_TRACE && (g_inj_cnt % g_burst_rate) == 0 && i != g_cnt)
...
u_int16_t
without_response(u_int32_t g_port_range)
{
    u_int64_t i;
    u_int32_t dstp = 0;
    ...
    for(i = 1; i < g_cnt + 1; i++)
    {
        ...
        if(g_verbose)
        {
            ...
            if(g_burst_rate != 0 && (g_inj_cnt % g_burst_rate) == 0 && i != g_cnt)
                sleep(g_interval_sec);
