...
libnet_t *shape_ipv4_hdr(libnet_t * g_pkt_d)
{
	...
	if (g_ip4hdr_o.s_addr == NULL) {
		if ((g_ip4hdr_o.n_saddr = libnet_get_ipaddr4(g_pkt_d)) == -1)
			fatal_error("Unable to retrieve local IP address: %s",
				    libnet_geterror(g_pkt_d));

		g_ip4hdr_o.s_addr =
		    (u_int8_t *) libnet_addr2name4(g_ip4hdr_o.n_saddr, 1);
	} else
	    if ((g_ip4hdr_o.n_saddr =
		 libnet_name2addr4(g_pkt_d, (char *)g_ip4hdr_o.s_addr,
				   1)) == -1)
	...
	if ((g_ip4hdr_o.n_daddr =
	     libnet_name2addr4(g_pkt_d, (char *)g_ip4hdr_o.d_addr, 1)) == -1)
		fatal_error("Invalid destination IP address: %s",
			    g_ip4hdr_o.d_addr);
	...
	if (libnet_build_ipv4(g_hdr_len,
			      g_ip4hdr_o.tos,
			      g_ip4hdr_o.id,
			      g_ip4hdr_o.frag,
			      g_ip4hdr_o.ttl,
			      ...
			      g_pkt_d, 0) == -1) {
		fatal_error("Unable to build IP header: %s",
			    libnet_geterror(g_pkt_d));
	}
